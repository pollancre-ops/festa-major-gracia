...
async function fetchCloudState(){
  const url=`${FAVORITES_API}?select=activity_id&group_id=eq.${encodeURIComponent(GROUP_ID)}`;const r=await fetch(url,{headers:authHeaders()});if(!r.ok)throw new Error('cloud fetch');const rows=await r.json();
  const cloudSaved=new Set(),cloudDone=new Set();let seedApplied=false;
  rows.forEach(x=>{const k=x.activity_id||'';if(k===YELLOW_SEED_MARKER){seedApplied=true;return}k.startsWith(DONE_PREFIX)?cloudDone.add(k.slice(DONE_PREFIX.length)):cloudSaved.add(k)});
  if(!localStorage.getItem(STATE_MIGRATION_KEY)){
    [...saved].forEach(id=>{cloudSaved.add(id);queueOp(id,'add')});[...done].forEach(id=>{cloudDone.add(id);queueOp(DONE_PREFIX+id,'add')});localStorage.setItem(STATE_MIGRATION_KEY,'1')
  }
  if(!seedApplied){
